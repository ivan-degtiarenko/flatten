...
func TestFlattenArraysIncluded(t *testing.T) {
	cases := []struct {
		test   string
...
func TestFlattenStringArraysIncluded(t *testing.T) {
	cases := []struct {
		test   string
		want   string
		prefix string
...
func TestFlattenArraysExcluded(t *testing.T) {
	cases := []struct {
		test   string
		want   map[string]interface{}
		prefix string
	...
	for i, test := range cases {
		...
		got, err := Flatten(m.(map[string]interface{}), test.prefix, test.style, false)
		if err != nil {
			t.Errorf("%d: failed to flatten: %v", i+1, err)
			continue
		}
...
func TestFlattenStringArraysExcluded(t *testing.T) {
	cases := []struct {
		test   string
		want   string
		prefix string
	...
	for i, test := range cases {
		got, err := FlattenString(test.test, test.prefix, test.style, false)
		if err != test.err {
			t.Errorf("%d: error mismatch, got: [%v], wanted: [%v]", i+1, err, test.err)
			continue
		}
